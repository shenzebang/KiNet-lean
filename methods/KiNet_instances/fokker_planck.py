...
def value_and_grad_fn(forward_fn, params, time_interval, data, rng, config, pde_instance: FokkerPlanck):
    ...
    def ode_func1(states, t):
        ...
        def g_t(xi, z):
            ...
            return jnp.mean(jnp.sum((f_t_theta_in_2 - xi) ** 2, axis=(1,)))
    ...
    def ode_func2(states, t):
        ...
        def g_t(xi, z, theta):
            ...
            # score_x, score_v = jnp.split(xi, indices_or_sections=2, axis=-1)
            return jnp.mean(jnp.sum((f_t_in_2_in_3 - xi) ** 2, axis=(1,)))
...
def distance_to_equilibrium(data, pde_instance: FokkerPlanck, rng):
...
def test_fn(forward_fn, data, time_interval, pde_instance: FokkerPlanck, rng):
    time_offset = time_interval["current"][-1] * len(time_interval["previous"])
...
def velocity_field_pretraining(pde_instance: FokkerPlanck, net, params):
